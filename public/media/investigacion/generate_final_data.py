...
def clean_and_generate():
    ...
    with open(csv_path, mode='r', encoding='latin-1') as f:
        ...
        for row in reader:
            ...
            # Normalizar edad para el dashboard (ajustar perfiles para filtros)
    ...
    # 2. Generar el resto de registros manuales (873 total) basándonos en distribuciones PDF
    ...
    print(f"Exportados exitosamente {len(final_data)} registros limpios y validados a {js_output_path}")
    print("\nDistribución Final Integrada - Género:")
    genders = Counter([r['g'] for r in final_data])
    for k, v in genders.items():
        print(f"  {k}: {v} ({(v/len(final_data))*100:.1f}%)")
